...
if a==True:
    ...
    opt_TM_cleaned_list = [x for x in df["TM Model"].unique() if x == x]
    opt_TM=st.selectbox(label='TM Model', options=opt_TM_cleaned_list)
    df1=df.loc[(df["TM Model"]==opt_TM)] # filter the data set
    
    
    # fro selecting the failure mode
    opt_failure_cleaned_list = [x for x in df1["Failure Mode -Primary (Part Name -1)"].unique() if x == x]
    opt_failure=st.selectbox(label='Failure Mode', options=opt_failure_cleaned_list,placeholder="Select an option")
    df2=df1.loc[df1["Failure Mode -Primary (Part Name -1)"]==opt_failure] # filter the data set
    
               
    # fro selecting the final gear ratio
    opt_Fgear_ratio_cleaned_list = [x for x in df2[" Final Gear Ratio"].unique() if x == x]
    opt_Fgear_ratio=st.selectbox(label='Final Gear Ratio', options=opt_Fgear_ratio_cleaned_list)
    df3=df2.loc[df2[" Final Gear Ratio"]==opt_Fgear_ratio]  # filter the data set
    
    print(df3.columns)
    for val in  df3.index:
        if isinstance(df3["Cycle (No's)"][val], str):
            try:
                df3["Cycle (No's)"][val]=  df3["Cycle (No's)"][val].replace('*','')
            except:
                pass
